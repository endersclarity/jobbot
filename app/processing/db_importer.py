...
class DatabaseImporter:
    ...
    def bulk_import_jobs(self, processed_job_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            jobs = data.get("jobs", [])
            if not jobs:
                return {"status": "no_data", "message": "No jobs found in processed file"}
        ...
        try:
            ...
            for i in range(0, len(jobs), batch_size):
                ...
                logger.info(
                    f"Batch {i//batch_size + 1}: {batch_results['imported']} imported, {batch_results['updated']} updated"
                )
            ...
            import_results["success_rate"] = (
                round(
                    (import_results["jobs_imported"] + import_results["jobs_updated"])
                    / import_results["total_jobs"]
                    * 100,
                    2,
                )
                if import_results["total_jobs"] > 0
                else 0
            )

            logger.info(
                f"Import complete: {import_results['jobs_imported']} imported, {import_results['jobs_updated']} updated"
            )
    ...
    def update_existing_jobs(self, job_updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        update_results = {"total_updates": len(job_updates), "successful_updates": 0, "failed_updates": 0, "errors": []}
        ...
        try:
            for update_data in job_updates:
                try:
                    ...
                    update_results["failed_updates"] += 1
                    update_results["errors"].append({"job_id": update_data.get("id"), "error": str(e)})
            ...
            if resolver:
                try:
                    ...
                    resolution_results["resolution_details"].append(
                        {"conflict": conflict, "resolved": False, "error": str(e)}
                    )
            else:
                resolution_results["unresolved"] += 1
    ...
    def _import_job_batch(self, job_batch: List[Dict[str, Any]], batch_id: str) -> Dict[str, Any]:
        ...
        batch_results = {"imported": 0, "updated": 0, "skipped": 0, "conflicts": 0, "errors": []}
        ...
        for job_data in job_batch:
            try:
                ...
                batch_results["errors"].append({"job_title": job_data.get("title", "Unknown"), "error": str(e)})
                batch_results["skipped"] += 1
    ...
    def _should_update_job(self, existing_job: Job, new_data: Dict[str, Any]) -> bool:
        ...
        existing_data_completeness = sum(
            1 for field in existing_job.__dict__.values() if field and str(field).strip() and field != "id"
        )
