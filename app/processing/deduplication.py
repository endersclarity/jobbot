...
class DuplicateDetector:
    ...
    def remove_duplicates_from_batch(
        self, job_list: List[Dict[str, Any]]
        ...
        stats = {
            ...
            "duplicate_rate": round((original_count - final_count) / original_count * 100, 2)
            if original_count > 0
            else 0,
        }

        logger.info(
            f"Deduplication complete: {original_count} → {final_count} jobs ({stats['duplicate_rate']}% duplicates)"
        )
