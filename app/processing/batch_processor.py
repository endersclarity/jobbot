...
class BatchProcessor:
    ...
    def process_file_queue(self, file_list: List[str], batch_name: str = None) -> Dict[str, Any]:
        ...
        if all_jobs:
            ...
            batch_results['normalization_stats'] = {
                ...
                'normalization_success_rate': round(len(normalized_jobs) / len(clean_jobs) * 100, 2) if clean_jobs else 0
            }
        ...
        logger.info(f"Batch processing complete: {batch_results['total_jobs_input']} → {batch_results['total_jobs_output']} jobs")
    ...
    def process_all_pending(self) -> Dict[str, Any]:
        ...
        if not pending_files:
            return {
                ...
                'message': 'All raw files have already been processed'
            }
        
        return self.process_file_queue(
            [str(f) for f in pending_files], 
            batch_name=f"pending_all_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def generate_processing_report(self, batch_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report = {
            ...
            'job_statistics': {
                ...
                'duplicates_removed': batch_results.get('total_jobs_input', 0) - batch_results.get('total_jobs_output', 0),
    ...
    def _generate_error_recommendations(self, error_categories: Dict[str, List]) -> List[str]:
        ...
        if error_categories['file_parsing_errors']:
            recommendations.append("Check raw file formats and encoding. Consider adding more robust file type detection.")
