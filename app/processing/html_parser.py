...
class JobDataExtractor:
    ...
    def extract_from_indeed_html(self, raw_html: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            job_containers = soup.find_all(['div'], attrs={
                'class': re.compile(r'job_seen_beacon|slider_container|jobsearch-SerpJobCard')
            })
            
            if not job_containers:
                # Fallback: look for any div with job-related data attributes
                job_containers = soup.find_all(['div'], attrs={
                    'data-jk': True  # Indeed job key attribute
                })
    ...
    def _extract_job_from_container(self, container) -> Optional[Dict[str, Any]]:
        ...
        # Extract title
        title_elem = container.find(['h2', 'a'], attrs={
            'data-jk': True
        }) or container.find(['span'], attrs={
        ...
        # Extract company
        company_elem = container.find(['span', 'a'], attrs={
            'class': re.compile(r'companyName|company')
        })
        if company_elem:
            job_data['company'] = company_elem.get_text(strip=True)
        
        # Extract location
        location_elem = container.find(['div', 'span'], attrs={
            'class': re.compile(r'companyLocation|location')
        })
        if location_elem:
            job_data['location'] = location_elem.get_text(strip=True)
        
        # Extract salary if present
        salary_elem = container.find(['span', 'div'], attrs={
            'class': re.compile(r'salary|estimated-salary')
        })
        if salary_elem:
        ...
        summary_elem = container.find(['div'], attrs={
            'class': re.compile(r'summary|job-snippet')
        })
        if summary_elem:
