...
class Application(Base):
    ...
    personal_interest_rating = Column(
        Integer, CheckConstraint("personal_interest_rating >= 1 AND personal_interest_rating <= 10"), nullable=True
    )
    compensation_expectation = Column(Integer, nullable=True)
    notes = Column(Text, nullable=True)
    exaggeration_level = Column(
        Integer, CheckConstraint("exaggeration_level >= 1 AND exaggeration_level <= 5"), nullable=True
    )  # 1=truthful, 5=heavily exaggerated
...
class EmployerResponse(Base):
    ...
    )  # acknowledgment, rejection, interview_request, offer, ghosted
...
class Reference(Base):
    ...
    credibility_rating = Column(
        Integer, CheckConstraint("credibility_rating >= 1 AND credibility_rating <= 10"), nullable=True
    )
...
class ExperienceClaim(Base):
    ...
    proficiency_claimed = Column(
        Integer, CheckConstraint("proficiency_claimed >= 1 AND proficiency_claimed <= 10"), nullable=True
    )
    actual_proficiency = Column(
        Integer, CheckConstraint("actual_proficiency >= 1 AND actual_proficiency <= 10"), nullable=True
    )
